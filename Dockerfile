...
# Install browsers and build tools with architecture-aware configuration
RUN apt-get update \
    && apt-get install -y gnupg wget ca-certificates --no-install-recommends \
    && if [ "$(uname -m)" = "aarch64" ]; then \
        # ARM64 specific installation (using Chromium)
        apt-get install -y chromium chromium-driver \
        && chromium --version && chromedriver --version; \
    else \
        # AMD64 specific installation (using Chrome)
    ...
    && rm -f geckodriver-v0.33.0-linux*.tar.gz \
